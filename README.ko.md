...
아래 샘플은 서로 다른 원본 참고 이미지를 사용했습니다. Panel 149이 각 이미지를 독립적으로 한 번만 생성했으며 AI 메타데이터도 제거했습니다. 가로 샘플은 왼쪽 원본·오른쪽 디자인의 정확한 50:50, 세로 샘플은 위 원본·아래 디자인의 정확한 50:50입니다.
...
- `design-only`: 전체 캔버스에 Panel 149의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-149`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-149 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-149-prompt.en.md)와 [중국어](references/xxd-panel-149-prompt.zh-CN.md)를 확인하세요.
